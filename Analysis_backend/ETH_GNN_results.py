...
import pandas as pd
import matplotlib
matplotlib.use("Agg")  # Non-GUI backend for FastAPI/macOS
import matplotlib.pyplot as plt
import os, json, pickle

from sklearn.linear_model import LinearRegression
from Analysis_backend import ETH_graph_analysis as notebook
...
def visualize_probability_analysis(predictions_df, threshold=0.7, save_path="backend/static/eth_gnn_forecast.png"):
    ...
    axes[0].legend(); axes[0].set_title('Congestion Probabilities'); axes[0].grid(True, alpha=0.3)
    ...
    axes[1].legend(); axes[1].set_title('Estimated Block Utilization'); axes[1].grid(True, alpha=0.3)
...
def run_eth_gnn_analysis(future_steps=10):
    ...
    model.load_state_dict(torch.load('Analysis_backend/best_hybrid_model.pth', map_location=device))
